...
public class TaskRestController {
    ...
    @GetMapping
    public ResponseEntity<PagedResponse<TaskDTO>> getAllTasks(@RequestParam(defaultValue = "0") Long page,
                                                              @RequestParam(required = false) Priority priority,
                                                              @RequestParam(required = false) TaskStatus status
                                                              ) {
        return ResponseEntity.ok(mapper.toPageDTO(service.getAllTasksByPage(page, priority, status)));
    }

    @GetMapping("/{id}")
    public ResponseEntity<TaskDTO> getTaskById(@PathVariable Long id) {
        TaskDTO task = mapper.toDTO(service.getTaskById(id));
    ...
    public ResponseEntity<TaskDTO> createTask(@Valid @RequestBody TaskDTO reqTask) {
        Task task = mapper.toEntity(reqTask);
        return new ResponseEntity<>(mapper.toDTO(service.createTask(task)), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<TaskDTO> updateTask(@PathVariable Long id,
        ...
        Task task = mapper.toEntity(reqTask);
        return ResponseEntity.ok(mapper.toDTO(service.updateTask(id, task)));
    }
